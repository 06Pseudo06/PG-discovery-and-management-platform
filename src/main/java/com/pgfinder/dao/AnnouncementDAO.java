...
public class AnnouncementDAO {
    ...
    public int insert(Connection conn, int pgId, int ownerId, String title, String message) throws SQLException {
        String sql = """
                INSERT INTO announcements (pg_id, owner_id, title, message)
                VALUES (?, ?, ?, ?)
                """;
    ...
    public List<Announcement> findForStudent(int studentId) {
        String sql = """
                SELECT DISTINCT a.*, p.name AS pg_name
                FROM announcements a
                JOIN pgs p ON a.pg_id = p.id
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            ps.setString(2, BookingStatus.CONFIRMED);
            ...
            throw new RuntimeException("Error loading student announcements.", e);
        }
